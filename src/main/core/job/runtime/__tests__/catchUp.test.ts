...
describe('computeCatchUpAction — fires stranded by a pause', () => {
  // Pause / resume stamp updatedAt but never advance nextRun / lastRun, so an
  // expected fire stays stranded in the past. A pause is one of the three ways
  // the reported regression loses a fire, so that fire is still owed —
  // JobManager settles it on resume.
  it('owes a cron fire that fell inside a pause the user has since lifted', () => {
    const schedule = makeSchedule({
      trigger: { kind: 'cron', expr: '0 9 * * *' },
      catchUpPolicy: { kind: 'after-startup', minutes: 1 },
  ...
  it('owes an interval fire that fell inside a pause the user has since lifted', () => {
    const schedule = makeSchedule({
  ...
  it('owes a never-fired cron whose first occurrence fell inside a lifted pause', () => {
    const schedule = makeSchedule({
      trigger: { kind: 'cron', expr: '0 9 * * *' },
      catchUpPolicy: { kind: 'after-startup', minutes: 1 },
      nextRun: null,
